...
def test_loaders__yaml__multiple__response_ref(fx_spec_required, fx_spec_as_file):
    ...
    fx_spec_required['paths']['/required-endpoint'] = {
        '$ref': f'{endpoint_spec_file.name}#/endpoint'
    }
    spec_file = fx_spec_as_file(fx_spec_required)
    spec_obj = load_from_yaml(spec_file)

    assert spec_obj['paths']['/required-endpoint'].get('$ref') is None
    assert spec_obj['paths']['/required-endpoint']['get']['responses']['200']['content'][
        'application/json'
